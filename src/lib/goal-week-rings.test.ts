...
// Rings returned (oldest → newest), including current in-progress week:
//   [0] 2024-05-06 – 2024-05-12
//   [1] 2024-05-13 – 2024-05-19
//   [2] 2024-05-20 – 2024-05-26
//   [3] 2024-05-27 – 2024-06-02
//   [4] 2024-06-03 – 2024-06-09
//   [5] 2024-06-10 – 2024-06-16  ← current in-progress week
...
describe("buildWeekRings", () => {
  ...
  it("returns exactly weekCount rings, oldest first, current week last", () => {
    const rings = buildWeekRings(DAILY);
    expect(rings).toHaveLength(6);
    expect(rings[0].weekStart).toBe("2024-05-06");
    expect(rings[5].weekStart).toBe(TODAY); // current in-progress week
  });

  it("respects weekCount override", () => {
    const rings = buildWeekRings({ ...DAILY, weekCount: 4 });
    expect(rings).toHaveLength(4);
    expect(rings[3].weekStart).toBe(TODAY);
  });

  it("includes the current in-progress week as the last ring", () => {
    const rings = buildWeekRings(DAILY);
    expect(rings[5].weekStart).toBe(TODAY);
  });
  ...
  it("marks weeks whose end date is before goalStartDate as not-started", () => {
    ...
    expect(rings[0].state).toBe("not-started"); // May 6–12 ends before May 20
    expect(rings[1].state).toBe("not-started"); // May 13–19 ends before May 20
    expect(rings[2].state).not.toBe("not-started"); // May 20–26 — goal started
  });
  ...
  it("done overrides skip: a week with both done and skip is not skipped", () => {
    ...
    expect(rings[4].state).toBe("partial");
  });
  ...
  it("partial (extra-only) tooltip mentions 'extra check-in' without 'only'", () => {
    const rings = buildWeekRings({ ...MON_ONLY, doneDates: ["2024-06-04"] });
    expect(rings[4].tooltip).toContain("1 extra check-in");
    expect(rings[4].tooltip).not.toContain("only");
  });
  ...
  it("partial (extra-only): extraDone=2 gives correct tooltip", () => {
    ...
    expect(rings[4].tooltip).toContain("2 extra check-ins");
    expect(rings[4].tooltip).not.toContain("only");
  });
  ...
  it("frequency goal: partial when scoredDone < weeklyTarget", () => {
    ...
    expect(rings[4].state).toBe("partial");
    expect(rings[4].completionRate).toBeCloseTo(1 / 3);
  });
  ...
  it("met when all 7 eligible days done on daily goal", () => {
    ...
    expect(rings[4].state).toBe("met");
    expect(rings[4].completionRate).toBe(1);
  });
  ...
  it("extra when met target AND off-target extras present", () => {
    ...
    expect(rings[4].state).toBe("extra");
    expect(rings[4].tooltip).toContain("1 extra");
  });
  ...
  it("frequency: extra when over-quota dones (4 done on 3x/week goal)", () => {
    ...
    expect(rings[4].state).toBe("extra");
    expect(rings[4].completionRate).toBe(1);
    expect(rings[4].tooltip).toContain("extra");
  });
  ...
  it("only counts check-ins in the correct week boundary", () => {
    // Dones only in rings[1] (May 13–19)
    const doneDates = ["2024-05-13", "2024-05-14", "2024-05-15", "2024-05-16", "2024-05-17", "2024-05-18", "2024-05-19"];
    const rings = buildWeekRings({ ...DAILY, doneDates });
    expect(rings[1].state).toBe("met");
    expect(rings[0].state).toBe("empty"); // May 6–12
    expect(rings[2].state).toBe("empty"); // May 20–26
  });

  it("skip dates in one week do not bleed into adjacent weeks", () => {
    const rings = buildWeekRings({ ...DAILY, skipDates: ["2024-06-03"] });
    expect(rings[4].state).toBe("skipped"); // Jun 3–9
    expect(rings[3].state).toBe("empty");   // May 27–Jun 2
  });
});
